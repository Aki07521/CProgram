...
/*
    ...
    业务注意：
    系统至少要保留一个管理员账号，否则后续无法进入后台管理菜单。

    链表语法点：
    删除用户时使用 prev 和 user 两个指针。
    prev 保存前一个节点，user 保存当前节点，删除时通过 prev->next 跳过当前节点。
*/
...
/*
    ...
    业务注意：
    未结账订单中仍然使用的菜品不能删除，因为订单明细、小票和结账流程还需要它。

    学习重点：
    修改菜品价格只影响之后的新订单，历史订单明细中保存的是“下单时价格”。
*/
...
/*
    ...
    业务注意：
    强制释放餐桌会取消未结账订单，并通过 cancelOrder 回滚库存。
    这个功能适合处理误操作或演示数据异常，不是普通结账流程。
*/
...
/*
    ...
    业务特点：
    该函数只读数据，不修改订单状态、不释放餐桌、不改变库存。

    学习重点：
    showReceiptForOrder 是展示小票的通用函数，结账和订单查询都会复用它。
*/
void showOrderQuery() {
    ensureDataLoaded();
...
/*
    ...
    统计规则：
    - 只统计 ORDER_PAID 已结账订单。
    - ORDER_ACTIVE、ORDER_WAIT_PAY、ORDER_CANCELLED 不计入营业额。
    - 菜品销量从已结账订单明细中重新累加，保证和历史小票一致。
...
/*
    ...
    业务注意：
    重置演示数据会清空当前链表，再重新 seedUsers、seedDishes、seedTables。
*/
